...
2.  What does the intercept tell you, in words?
    ...
    -   for a car with a engine size of 0, when x2 = 0 (year = 2008), the engine size is 35.79

3.  What does the coefficient on `displ` tell you, in words?

    -   the relationship between hwy mileage and engine size is negative at -3.8

4.  What does the coefficient estimate on the 2008 vintage indicator variable (`year2008`) tell you, in words?

    2.  more cars with larger engines started being made in 2008 when compared to 1999
...
```{r}
ggplot(augment(hwy_lm), aes(x = displ, y = hwy, color = year)) +
  ...
  # (x2 = 1 OR year = 2008)
  geom_abline(slope = coef(hwy_lm)['displ'] + 
                coef(hwy_lm)['displ:year2008'], 
              intercept = coef(hwy_lm)['(Intercept)'] +
                coef(hwy_lm)['year2008'],
              color = 'brown')

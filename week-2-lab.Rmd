...
ggplot(crab_gb, aes(x = latitude, y = mean_size)) + 
  geom_col()
...
```{r}
cor(pie_crabs$latitude, pie_crabs$size)
```
...
```{r}
# b0 is intercept
#above equation is for population because it is hatless

summary(lm(pie_crabs$size ~ pie_crabs$latitude))

# intecept (b0) = -3.62
# slope (b1) = 0.485

# y intervept (beta hat 1) is negative and data might have negative size crabs
# therefore y intercept is not directly interpretable
```
...
ggplot(crab_gb, aes(x = latitude, y = mean_size)) + 
  geom_col() +
  geom_smooth(data = pie_crabs, mapping = aes(x = latitude, y = size), method = lm, se = FALSE)
```
...
1.  Add to the `pie_crab` dataframe a column of predicted total lengths $\hat y_i$ and a column of residuals $e_i = y_i-\hat y_i$.[^week-2-lab-4]

[^week-2-lab-4]: **Hint:** In `dplyr` you can use `add_predictions(mod)` where `mod` is a regression model to generate a new variable containing your model's predicted values.

```{r}
pie_crab2 <- pie_crab
crab_lm <- lm(size ~ latitude, pie_crab)
seq <- seq(from = 1, to = 100, by = 1)

pie_crab2 <- pie_crab2 %>%
  mutate(predicted_size = predict(crab_lm),
         size_residul = residuals(crab_lm))
...
3.  Compute the mean of your residuals. Is it basically zero?[^week-2-lab-5]

[^week-2-lab-5]: **Note:** OLS gives you this one for free. The OLS regression line will always go through the middle of your data, as it aims to minimize squared errors. So, you'll always end up with sample errors that are mean zero. This doesn't necessarily mean that in your population $u$ is mean zero!
...
ggplot(pie_crab2, aes(x = latitude, y = size_residul)) +
  geom_point() +

...
loans_sum <- loans %>% group_by(homeownership) %>% count()

# Bar plot showing proportion of loans in each homeownership category
ggplot(loans_sum, aes(x = homeownership, y = n)) +
  geom_col(fill = "slateblue4") +
  theme_bw()

# he was making a new bar graph like above but he was doing something with mutate, missed it
```
...
ggplot(loans) + 
  geom_histogram(aes(x = interest_rate)) + scale_x_continuous(breaks = breaks)

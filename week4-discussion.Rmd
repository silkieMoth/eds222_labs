...
prediction <- expand_grid( # get a df of simulated values for canopy_height and wolf_density
  canopy_height = seq(70, 130, length.out = 5),
  wolf_density = c(1.5, 3, 4.5)
) %>% 
  mutate(yhat = beta0_hat + beta1_hat * canopy_height + beta2_hat * wolf_density + #add column for regression eq
...
interpreting this graph - top line: elkdensity/canopyheight when wolf = 1.5 -

middle line: elkdensity/canopyheight when wolf = 3 -

bottom line: elkdensity/canopyheight when wolf = 4.5

Questions:

-   Where would the line for wolf density = 4 be? How about wolf density = 2?
    -   4 = lightest, lowest line, 2 = darkest, highest line
-   What's the slope of canopy height when wolf density is 1.5, 3, and 4.5?
    -   1.5, lower then darkest line, 4.5, higher then lighest line

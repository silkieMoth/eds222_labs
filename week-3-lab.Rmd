...
```{r}
# 1.
summary(lm(size ~ latitude, data=pie_crab))$r.squared
summary(lm(water_temp ~ latitude, data=pie_crab))$r.squared

sd(pie_crab$size)
sd(pie_crab$water_temp)

# 2.it fits water temp better. latitude ~ size has a lot more data points with a lot more variance, even though they have similar sd's, likely due to the different sample sizes, so this is about what I expected. If we were to group size by mean, the r2 goes up
```
...
```{r}
size_model <- summary(lm(size ~ latitude, pie_crab))
size_sse <- sum((resid(size_model))^2)
size_sst <- sum((pie_crab$size - mean(pie_crab$size))^2)
size_r2 <- 1- (size_sse/size_sst)
size_resid <- resid(size_model)
sum(size_resid)
```
...
```{r}
ggplot(mpg, aes(group = year, y = hwy)) +
  geom_boxplot()
```
...
# what this is doing
# when lm() recieves a categorical variable, it creates new boolean variables under the hood for whether or not the categorical variable = each category
#y intercept is the mean of the reference level
# the reference level being the category the lm() selected out, in this case being 1999
# the slope is the difference between the mean of the two categories
car_lm <- lm(hwy ~ year_fct, mpg)
...
```{r}
# multiple regression looks like
hwy_lm <- lm(hwy ~ year_fct + displ, data = mpg)

summary(hwy_lm)

# how to interpret
# adding displ shifts the line up or down based on categorical variable
# confounding variable is how much line has shifted
# kind of idk

# a engine in 2008 is 1.4 times more efficient than an engine in 1999
```
...
hwy_lm <- lm(paste("hwy", "~ year_fct", " + displ"), filter(.data = mpg, year_fct == 1999))

paste(hwy, "~ year_fct", " + displ")
